...
$(document).ready(function () {
  ...
  function initializeChart(labels, transactions, earnings, revenue) {
    ...
    // reapply any series the user had hidden before this rebuild
  ...
  // ignore clicks until chart exists, so the box state and series state can't go out of sync
  ...
  $('.sidebar-button').click(function () {
    const buttonName = $(this).data('page');
    if (buttonName === selectedPage) return;
    selectPage(buttonName);
  });

  function selectPage(page) {
    $('.sidebar-button.active').removeClass('active');
     $(`.sidebar-button[data-page="${page}"]`).addClass('active');
    selectedPage = page;
  }
});